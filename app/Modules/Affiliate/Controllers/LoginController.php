...
class LoginController extends Controller
{
    ...
    */
    ...
    {
        $this->middleware('affiliate.guest')->except('signout');
        $this->email_service = $email_service;
    }

    public function showSigninForm(){
        return view('Affiliate::auth.signin');
    }
    ...
    public function signin(Request $request){
     ...
     //dd(md5($request->password) , $request->email);
        $userObj = User::whereEmail($request->email)
                    ...
                    ->whereRaw(" ( user_type = 'AFFILIATE' ) ")
                    ->first();

      // dd($userObj);
        if ( empty($userObj)  ) {
           // $this->incrementLoginAttempts($request);
            return redirect(route("affiliate.signin"))->with('error', 'invalid email/password provided.');
        }
       ...
       // $this->clearLoginAttempts($request);
        \Auth::guard('affiliate')->login($userObj, $remember);
        return redirect(route('affiliate.dashboard'));
    }


    public function showForgotPasswordForm(){
        return view('Affiliate::auth.forgot_password');
    }
    ...
    public function signout(){
        ...
        return redirect(route('affiliate.signin'));
    }
